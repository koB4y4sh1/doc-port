...
import { BookOpen, ChevronRight, Clock, List, Menu, X } from 'lucide-react';
import {
  ARTICLE_ROOT_CLASS,
  sanitizeArticleHtml,
  validateAndScopeCss,
} from './lib/articleSecurity';
import { cn } from './lib/utils';

const rawArticles = (import.meta as any).glob('./pages/*.html', {
  query: '?raw',
  import: 'default',
  eager: true,
});
...
const parseArticles = (): Article[] => {
  return Object.entries(rawArticles).map(([path, content], index) => {
    ...
    const sanitizedContent = sanitizeArticleHtml(doc.body.innerHTML);
...
const publishedArticles = articles.filter((article) => !article.blocked);
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="fixed top-0 left-0 bottom-0 w-80 bg-navy-900 border-r border-navy-800 z-50 shadow-2xl shadow-black/50"
      >
...
const ArticlePage = () => {
  ...
  useEffect(() => {
    if (!article?.styles || article.blocked) {
      return;
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-4">
        ...
        <p className="text-sm text-slate-500 max-w-lg text-center">
          This article contains disallowed HTML or CSS and is not published.
        </p>
        <div className="w-full max-w-2xl bg-navy-900 border border-navy-800 rounded-2xl p-6">
          <ul className="space-y-2 text-xs text-rose-300">
            {article.validationErrors.map((error) => (
